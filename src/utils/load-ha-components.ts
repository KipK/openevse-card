...
export const loadHaComponents = async (): Promise<void> => {
    try {
        ...
        if (
            REQUIRED_HA_COMPONENTS.every((component) =>
            ...
            new Promise((_, reject) =>
                setTimeout(
                    () =>
                        reject(
                            new Error(
                                'Timeout waiting for partial-panel-resolver'
                            )
                        ),
                    10000
                )
        ...
        if (typeof ppr._updateRoutes !== 'function') {
            throw new Error(
                'partial-panel-resolver does not have _updateRoutes method'
            );
        }
        ...
        if (!ppr.routerOptions?.routes?.tmp?.load) {
            throw new Error(
                'Failed to create tmp route in partial-panel-resolver'
            );
        }
        ...
        await Promise.race([
            ...
            new Promise((_, reject) =>
                setTimeout(
                    () => reject(new Error('Timeout loading tmp route')),
                    10000
                )
            ),
        ]);
        ...
        await Promise.race([
            ...
            new Promise((_, reject) =>
                setTimeout(
                    () =>
                        reject(
                            new Error('Timeout waiting for ha-panel-config')
                        ),
                    10000
                )
            ),
        ...
        await Promise.race([
            ...
            new Promise((_, reject) =>
                setTimeout(
                    () =>
                        reject(
                            new Error('Timeout loading automation components')
                        ),
                    10000
                )
            ),
        ...
        if (missingComponents.length > 0) {
            throw new Error(
                `Failed to load components: ${missingComponents.join(', ')}`
            );
        }
    } catch (error) {
        ...
        try {
            ...
            if (
                ...
                console.log(
                    'Attempting fallback loading method for HA components'
                );
