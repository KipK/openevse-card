...
class CustomCardEditor extends LitElement {
    ...
    _isEntityConfigured(configKey) {
        return Boolean(this.config[configKey] &&
            this.config[configKey].length > 0);
    }
    ...
    async _loadDeviceEntities(deviceId) {
        ...
        const deviceEntitiesList = entityRegistry.filter(entity => entity.device_id === deviceId);
        ...
        for (const [configKey, mapping] of Object.entries(entityMappings)) {
            ...
            let matchedEntity = deviceEntitiesList.find(entity => entity.entity_id.toLowerCase() === preferredPattern.toLowerCase());
            ...
            if (!matchedEntity) {
                matchedEntity = deviceEntitiesList.find(entity => {
                    ...
                    return names.some(name => entityId.includes(name.toLowerCase()) ||
    ...
    _updateOptionalEntity(index, changedValues) {
        ...
        this.optionalEntities = this.optionalEntities.map((entity, i) => i === index ? updatedEntity : entity);
    ...
    _getMissingEntities() {
        ...
        return requiredEntities.filter(entity => {
            const isInConfig = this.config[entity] &&
                this.config[entity].length > 0;
            const isInDetected = this.openEVSEEntities[entity] &&
                this.openEVSEEntities[entity].length > 0;
    ...
    render() {
        ...
        if (this.config.device_id && this.hass.entities) {
            ...
            const deviceEntityList = entityRegistry.filter(entity => entity.device_id === this.config.device_id);
...
//# sourceMappingURL=custom-card-editor.js.map